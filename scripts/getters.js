...
Tabulous.prototype.getVoicings = function(startingFret, voicings){
	...
	// fitness function aka algorithm to pair down
		...
		// playability: higher notes on higher strings, lower notes on lower strings

	// console.log(population);

	switch(this.settings.algorithm) {
		case 'KORDFU':
			return this.getVoicingsKORDFU(startingFret, voicings);
			break;
		case 'CHAIN':
...
Tabulous.prototype.getPopulation = function(startingFret, voicings){
	...
	return true === cont ? this.getPopulation(startFret, voicings) : this.filterDupVoicings(voicings);

};

Tabulous.prototype.getVoicingsKORDFU = function(startingFret, voicings){
	...
	var prevLastFret = voicings.length > 0 ? _.chain(_.last(voicings).voicing).compact().sortBy().last().value() : 0;
	var lastFret     = _.chain(tab).compact().sortBy().last().value() // get current last fret
	var filteredTab	 = _.chain(tab).compact().value().filter(function(t){ return t !== -1 }); // rip out open frets and -1s
	var startFret    = prevLastFret < 12 && lastFret > 12 ? 12 : lastFret; // if we just passed the 12th fret, reset algo starting fret
		startFret    = _.contains(tab, 0) ? filteredTab.sort()[0] : startFret; // if we used an open string, reset starting fret to first fret used
	var cont         = (startingFret + this.settings.span) < this.settings.frets; // continue if we have more frets to walk
	...
	return true === cont ? this.getVoicingsKORDFU(startFret, voicings) : voicings;
